...
//        $GameModel->keyword=$data['keyword'];
        ...
        if($value){
            LogAction::logAction("添加游戏[".$data['game_name']."]成功");
            return ['msg'=>"添加游戏成功",'error'=>true];
        }else{
            LogAction::logAction("添加游戏[".$data['game_name']."]失败");
            return ['msg'=>"添加游戏失败",'error'=>false];
        }
    }
    ...
    static public function updateGame($data)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $result =DB::table('game_lists')
                ...
                ->update($data);
            if ($result) {
                LogAction::logAction("修改游戏[".$data['game_name']."]成功");
                return ['msg' => "修改成功", 'error' => true];
            } else {
                LogAction::logAction("修改游戏[".$data['game_name']."]失败");
